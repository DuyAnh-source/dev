...
def send_movement_pos_ned(origin_ned={'x': 0, 'y': 0, 'z': 0}, x=0, y=0, altitude=0, yaw=0.0):
    master.mav.set_position_target_local_ned_send(
        px4.get_synced_time_boot_ms(),
        master.target_system,
        master.target_component,
        mavutil.mavlink.MAV_FRAME_LOCAL_NED,
        0b100111111000,  # bật điều khiển vị trí x, y, z và yaw
        origin_ned['x'] + x,
        origin_ned['y'] + y,
        origin_ned['z'] - altitude,
        0, 0, 0,               # không điều khiển velocity
        0, 0, 0,               # không điều khiển acceleration
        yaw, 0                 # yaw (radian), yaw_rate = 0
    )


for _ in range(20):  # gửi trong ~1s 
    send_movement_pos_ned(origin_ned = origin_ned,x = 0,y = 0,altitude = 1,yaw=0.0)
...
def thread_send_commands():
    ...
    while not stop_event.is_set():
        yaw = yaw + 0.005
        send_movement_pos_ned(origin_ned = origin_ned,x = 0,y = 0,altitude = 1,yaw=yaw)
        time.sleep(0.05)
def thread_read_msg():
    while not stop_event.is_set():
