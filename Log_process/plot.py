...
script_dir = os.path.dirname(os.path.realpath(__file__))  # thư mục của file .py
csv_path = os.path.join(script_dir, "ulog_extracted", "trajectory_data.csv")
df = pd.read_csv(csv_path)

# === Vẽ 1: Quỹ đạo bay theo mặt phẳng XY ===
plt.figure()
plt.plot(df['y'], df['x'])
plt.scatter(df['y'][0], df['x'][0], color='red', s=100, label='Điểm bắt đầu', zorder=5)
plt.xlabel("Y (m)")
plt.ylabel("X (m)")
plt.title("Quỹ đạo bay (XY)")
plt.axis('equal')
plt.grid()
